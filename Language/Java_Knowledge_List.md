...
## 修饰符: [Java关键字final、static使用总结](http://lavasoft.blog.51cto.com/62575/18771)
**final**
...
4.下面属于java包装类的是？  
A.String  B.Boolean  C.Character  D.Double
...
1. 是否可以继承String类?  A:可以  B:不可以
2. 程序中可以允许多个类同时拥有都有main方法吗?  A:允许  B:不允许
3. sizeof是Java的关键字吗?  A:是  B:不是
4. short s1 = 1; s1 = s1 + 1;有错吗?  A:有错  B:没有错
5. short s1 = 1; s1 += 1;有错吗?  A:有错  B:没有错
6. Swing 是线程安全的?  A:是  B:不是
7. 我们能创建一个包含可变对象的不可变对象吗?  A:能  B:不能
8. Java 中 ++ 操作符是线程安全的吗?  A:是  B:不是

**简答题** 

1.请写出Java中的基本数据类型以及他们各自的包装类型 
2.在Java中有四种不同的作用域:public protected default private, 请在下表中指出各作用域的范围
|作用域 | 当前类 | 同一包内 | 子孙类 | 其他包
|:-----:|:-------|:---------|:-------|:-----
|public   |√ |√  |√  |√
|protected|
...
3.Java中如何实现同步? 请写出与线程同步以及线程调度相关的方法。  
4.怎么获取Java程序使用的内存？堆使用的百分比？  
5.用Java写一个线程安全的单例模式（Singleton）  
6.请写出一段线程安全的java代码，要求用户输入N行，每行M个整型数字，数字之间用空白符隔开，输出N行，每行输出1~N行中出现的不同数字的种数
...
### 笔试题答案
选择题：ACD,ACD,C,BCD,AC, AB,AB,C,C,C  
判断题：B A B A B B A B 
...
- notityAll()：唤醒所有处于等待状态的线程，而不是将对象的锁给所有线程，而是让它们竞争，只有获得锁的线程才能进入就绪状态；
...
public class Singleton{
    ...
    public void show(){ System.out.println("Singleon using static initialization");}
}
Singleton.getInstance().show(); // Here is how to access this Singleton class
```
